...
export default function StudentExamPage() {
  ...
  // Countdown timer
  useEffect(() => {
    if (!hasStarted || remainingTime === null || remainingTime <= 0) return;
  ...
  useEffect(() => {
    if (remainingTime === 0 && !submitted && !submitting) {
      autoSubmitExam();
    }
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen flex flex-col overflow-hidden bg-[#f4f7fb] text-slate-800 font-sans z-0">
      ...
      <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 sm:px-16 z-10 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <Clock className={`h-6 w-6 ${remainingTime !== null && remainingTime <= 300 ? 'text-red-500 animate-pulse' : 'text-blue-600'}`} />
          <div className="flex flex-col">
            <span className={`text-[10px] font-bold tracking-wider uppercase leading-none ${remainingTime !== null && remainingTime <= 300 ? 'text-red-400' : 'text-slate-400'}`}>
              Time Remaining
            </span>
            <span className={`text-2xl font-black font-mono tracking-tight mt-1 leading-none ${remainingTime !== null && remainingTime <= 300 ? 'text-red-600 animate-pulse' : 'text-blue-600'}`}>
          ...
          <div className="col-span-12 lg:col-span-8 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col justify-between overflow-hidden h-auto lg:h-full min-h-[500px] lg:min-h-0">
            ...
            <div className="p-8 space-y-6 flex-1 min-h-0 overflow-y-auto scrollbar-light bg-white">
              ...
              {activeQuestion.images && activeQuestion.images.length > 0 && (
                <div className={`grid gap-4 mt-4 ${activeQuestion.images.length === 1 ? 'grid-cols-1 max-w-lg' :
                ...
                {activeQuestion.options.map((opt: any) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all duration-300 ease-out transform ${isSelected
                        ? 'bg-blue-50 border-blue-500 shadow-md -translate-y-1 text-blue-900 font-bold'
                        : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50 hover:border-blue-300 hover:shadow-lg hover:-translate-y-1'
                        }`}
                    >
                      <div className="flex flex-col gap-3 w-full">
                        <div className="flex items-center gap-3">
                          <span className={`h-6 w-6 rounded-full flex items-center justify-center border-2 transition-all duration-300 shrink-0 ${isSelected 
                              ? 'border-blue-500 bg-white scale-110' 
                              : 'border-slate-300 bg-white group-hover:border-blue-300 group-hover:scale-105'
                            }`}>
            ...
            <div className="px-8 py-5 border-t border-slate-100 flex items-center justify-end gap-3 bg-white shrink-0">
              <button
                ...
                className="group px-6 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-500 hover:text-slate-800 disabled:opacity-30 disabled:hover:text-slate-500 transition-all duration-300 flex items-center gap-1.5 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 active:scale-95 cursor-pointer disabled:cursor-not-allowed disabled:transform-none disabled:shadow-sm"
              >
                <ArrowLeft className="h-4 w-4 transition-transform duration-300 group-hover:-translate-x-1" /> Previous
              </button>

              <button
                disabled={currentIdx === questions.length - 1}
                onClick={() => setCurrentIdx((prev) => prev + 1)}
                className="group px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all duration-300 shadow-md shadow-blue-600/20 hover:shadow-lg hover:shadow-blue-600/30 hover:-translate-y-0.5 active:translate-y-0 active:scale-95 disabled:opacity-30 disabled:hover:bg-blue-600 cursor-pointer disabled:cursor-not-allowed disabled:transform-none disabled:shadow-md"
              >
                Next <ArrowRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" />
              </button>
            </div>
          </div>

          {/* Right Column: Question Navigator */}
          <div className="col-span-12 lg:col-span-4 bg-white border border-slate-200 rounded-2xl shadow-sm p-5 lg:p-6 flex flex-col justify-between h-auto lg:h-full overflow-hidden">
            <div className="space-y-5 flex flex-col overflow-hidden min-h-0 flex-1">
              {/* Navigator Title & Progress */}
              <div className="flex flex-col gap-3">
                <div className="flex justify-between items-end mb-1">
                  <span className="text-base font-bold text-slate-900 tracking-tight">Question Navigator</span>
                ...
                {/* Legend */}
                <div className="flex gap-5 items-center text-[10px] font-bold text-slate-400 mt-2">
                  <div className="flex items-center gap-2">
                    <span className="h-3.5 w-3.5 rounded bg-emerald-500 shadow-sm" />
                    <span className="uppercase tracking-wider">Answered</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="h-3.5 w-3.5 rounded bg-slate-100 border border-slate-200 shadow-sm" />
                    <span className="uppercase tracking-wider">Not Answered</span>
                  </div>
                </div>
              </div>
                    ...
                    <button
                      ...
                      className={`w-full aspect-square rounded-lg font-bold text-[10px] flex items-center justify-center transition-all cursor-pointer select-none ${
                        isVisited
                          ...
                          : isAnswered
                            ? 'bg-emerald-500 text-white shadow-sm border border-emerald-500 hover:bg-emerald-400'
            ...
            <div className="space-y-4 mt-5 pt-4 border-t border-slate-100 shrink-0">
              {/* Sync Status & Metrics */}
              <div className="space-y-3">
                {/* Sync status */}
                <div className="flex items-center justify-between text-xs pb-2 border-b border-slate-50">
                  <span className="text-slate-400 font-semibold tracking-wide">Sync Status</span>
                  {isOnline ? (
                ...
                {pendingSync > 0 && (
                  <div className="p-3 bg-amber-50 border border-amber-200 text-amber-600 rounded-xl text-[10px] leading-tight font-bold">
                ...
                <div className="flex items-center justify-between text-xs font-semibold text-slate-500">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="tracking-wide">Answered</span>
                  </div>
                  <span className="text-slate-800 font-black text-sm">{answeredCount}</span>
                </div>
                ...
                <div className="flex items-center justify-between text-xs font-semibold text-slate-500">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="tracking-wide">Not Answered</span>
                  </div>
                  <span className="text-slate-800 font-black text-sm">{notAnsweredCount}</span>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-4 mt-2 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-xl text-sm transition-all duration-300 shadow-md shadow-emerald-600/20 hover:shadow-lg hover:shadow-emerald-600/40 hover:-translate-y-1 active:translate-y-0 active:scale-95 flex items-center justify-center gap-2 cursor-pointer group"
              >
                <ListTodo className="h-4 w-4 transition-transform duration-300 group-hover:scale-110" /> Review & Submit
              </button>
            </div>
          </div>
        </div>
